...
namespace ProjectManager.Server.Models {
    public class UserModel : IUser {
        ...
        public UserModel(ApplicationDbContext context, IUtility utilityService, UserManager<User> userManager) {
            ...
            User? currentUser = await _utilityService.GetUser() ?? throw new InvalidDataException("Invalid User.");
            User user = await _context.Users.Include("Companies.Company").SingleAsync(u => u.Id == currentUser.Id);
            return new UserDto(user);
        }

                                                        public async Task<UserDto> Get(String id) {
            User? user = await _context.Users.FirstOrDefaultAsync(user => user.Id == id) ?? throw new InvalidDataException("User not found.");
            return new UserDto(user);
        }
            ...
            return companies == null
                ? throw new InvalidDataException("Invalid CompanyID.")
                : companies.Users!.Select(user => new UserDto(user.User!)).ToList();
        }
            ...
            User? currentUser = await _utilityService.GetUser() ?? throw new InvalidDataException("Invalid User.");
            User? user = await _context.Users.FirstOrDefaultAsync(u => u.Email == userInvite.Email) ?? throw new InvalidDataException("User not found.");
            UserCompany? currentUserCompany = await _context.UserCompany.FirstOrDefaultAsync(u =>
                u.UserId == currentUser.Id && u.CompanyId == userInvite.CompanyId && u.Role == UserRole.Admin) ?? throw new UnauthorizedAccessException("User does not have required permission");
            ...
            switch (userCompany)
            {
                ...
                {
                    UserCompany newUserCompany = new()
                    {
                        UserId = user.Id,
            ...
            UserCompany? company = await _context.UserCompany.Include("Company")
                .FirstOrDefaultAsync(uc => uc.CompanyId == companyId && uc.UserId == currentUser.Id) ?? throw new InvalidDataException("Invalid CompanyId.");
            ...
            User? currentUser = await _utilityService.GetUser() ?? throw new InvalidDataException("Invalid User.");
            Project? project = await _context.Projects.Include("Users").FirstOrDefaultAsync(p => p.Id == projectId) ?? throw new InvalidDataException("Invalid ProjectId.");
            User? user = await _context.Users.FirstOrDefaultAsync(u => u.Id == userId) ?? throw new InvalidDataException("Invalid UserId.");
            ...
            return project == null
                ...
                : project.Users!.Select(user => new UserDto(user)).ToList();
        }
    }
}
